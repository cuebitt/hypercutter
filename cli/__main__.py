...
def prompt_clear_directory(path: Path) -> bool:
    ...
    while True:
        response = input("Clear output directory before writing? [y/N] ").strip().lower()
...
def main() -> None:
    ...
    if not sym_path:
        if identified:
            ...
            logging.info("Identified ROM: %s (%s)", identified.game.name, identified.sym_filename)
        elif args.game:
            ...
            game_code = rom_data[GAME_CODE_OFFSET : GAME_CODE_OFFSET + 4] if len(rom_data) >= GAME_CODE_OFFSET + 4 else b"????"
            logging.error(
